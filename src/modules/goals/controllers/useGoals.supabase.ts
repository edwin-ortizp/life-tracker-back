...
import { GoalsService } from '@/modules/goals/services';
import { useAuth } from '@/shared/hooks/useAuth';
import { generateTaskCode } from '@/shared/utils/taskCode';
import type { Goal, GoalEntry, GoalTask, GoalsHook, NumericEntry } from '../models';
...
export const useGoals = (): GoalsHook => {
  ...
  const loadGoals = useCallback(async () => {
    ...
    try {
      ...
      const { data: goalsData, error: goalsError } = await GoalsService.table('goals')
        .select('*')
        .eq('user_id', user.id);
      ...
      const safeSelectByGoalIds = async (table: string) => {
        ...
        const result = await GoalsService.table(table).select('*').in('goal_id', goalIds);
      ...
      (numericEntriesResult.data || []).forEach((ne: any) => {
      ...
      // Combine all data
      const list: Goal[] = (goalsData || []).map((row: any) => ({
        id: row.id,
        userId: row.user_id,
        title: row.title,
  ...
  const addGoal = async (goalData: Omit<Goal, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
    ...
    setError(null);
    try {
      const { data, error: insertError } = await GoalsService.table('goals')
        .insert({
          user_id: user.id,
  ...
  const updateGoal = async (id: string, data: Partial<Goal>) => {
    ...
    setGoals(prev => prev.map((g: any) => g.id === id ? updatedGoal : g));
    try {
      ...
      const { error: updateError } = await GoalsService.table('goals')
        .update(updateData)
        .eq('id', id);
      ...
      setGoals(prev => prev.map((g: any) => g.id === id ? original : g));
  ...
  const addTask = async (goalId: string, taskTitle: string) => {
    ...
    try {
      ...
      const { error: insertError } = await GoalsService.table('tasks')
        .insert({
          user_id: user.id,
          goal_id: goalId,
          task_code: taskCode,
  ...
  const toggleTask = async (goalId: string, taskId: string | undefined, taskIndex: number) => {
    ...
    try {
      ...
      if (!resolvedTaskId) {
        ...
        const { data: tasks } = await GoalsService.table('tasks')
          .select('id')
          .eq('user_id', user.id)
          .eq('goal_id', goalId)
          .eq('title', task.title)
      ...
      const { error: updateError } = await GoalsService.table('tasks')
        .update({
          completed: !task.done,
          updated_at: new Date().toISOString()
        })
  ...
  const deleteTask = async (goalId: string, taskId: string) => {
    ...
    try {
      const { error: deleteError } = await GoalsService.table('tasks')
        .delete()
        .eq('id', taskId)
        .eq('user_id', user.id);
  ...
  const addEntry = async (goalId: string, entry: Omit<GoalEntry, 'date'> & { date?: string }) => {
    ...
    try {
      const { error: insertError } = await GoalsService.table('goal_entries')
        .insert({
          goal_id: goalId,
          text: entry.text,
  ...
  const addNumericEntry = async (goalId: string, entry: Omit<NumericEntry, 'date'> & { date?: string }) => {
    ...
    try {
      const { error: insertError } = await GoalsService.table('goal_numeric_entries')
        .insert({
          goal_id: goalId,
          value: entry.value,
          date: entry.date || new Date().toISOString(),
