...
import { getCheckboxProgress } from '@/shared/utils/markdown';
...
export const useTaskData = () => {
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await TaskService.table('tasks')
        .select('*')
        .eq('user_id', user.id)
      ...
      const allTaskList = (data || []).map((row: any) => ({
        id: row.id,
  ...
  const getNextTaskCode = useCallback(async () => {
    ...
    reservedTaskCodesRef.current.forEach((code: any) => reservedCodes.add(code));
  ...
  const addTask = useCallback(async (formData: TaskFormData) => {
    ...
    try {
      ...
      const { data, error: insertError } = await TaskService.table('tasks')
        .insert(taskData)
        .select()
        .single();
  ...
  const editTask = useCallback(async (taskId: string, updates: Partial<TaskFormData>) => {
    ...
    try {
      ...
      const { error: updateError } = await TaskService.table('tasks')
        .update(updateData)
        .eq('id', taskId);
  ...
  const deleteTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      const { error: deleteError } = await TaskService.table('tasks')
        .delete()
        .eq('id', taskId);
  ...
  const completeRecurrentTask = useCallback(async (data: TaskFormData) => {
    ...
    try {
      ...
      const { data: insertedData, error: insertError } = await TaskService.table('tasks')
        .insert(nextTaskData)
        .select()
        .single();
