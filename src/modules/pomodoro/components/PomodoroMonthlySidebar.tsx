...
import { usePomodoroMonthlyProgress, type DailyPomodoroAggregate } from '../controllers/usePomodoroMonthlyProgress';

interface PomodoroMonthlySidebarProps {
  selectedDate: Date;
...
interface CalendarCell {
  ...
  isFuture: boolean;
  dayNumber: number;
  aggregate: DailyPomodoroAggregate;
}
...
const EMPTY_DAY: DailyPomodoroAggregate = {
  date: '',
  workedMinutes: 0,
  entries: 0,
  goalMinutes: 0,
...
export const PomodoroMonthlySidebar: React.FC<PomodoroMonthlySidebarProps> = ({
  selectedDate,
  onDateChange,
  goalMinutes
}) => {
  ...
  } = usePomodoroMonthlyProgress(visibleMonth, goalMinutes);
  ...
  }, [dailyStatsMap, monthEnd, monthStart, resolvedGoalMinutes, todayKey, visibleMonth]);
  ...
  const stats = useMemo(() => {
    ...
    const averageMinutes = workDays > 0 ? Math.round(totalMinutes / workDays) : 0;
      ...
      <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-7 gap-2">
            {calendarCells.map((cell) => {
              ...
              if (cell.aggregate.entries > 0) {
                strokeColor = cell.aggregate.percent > 100 ? '#ff6b00' : '#f08c3d';
              }
              ...
              return (
                <Tooltip key={cell.dateKey}>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        ...
                        cell.isToday && 'bg-orange-500/10 ring-1 ring-orange-500/25',
                  ...
                  <TooltipContent side="left" className="max-w-[220px] space-y-1 text-sm">
                    ...
                    <p>{cell.aggregate.workedMinutes} / {cell.aggregate.goalMinutes} min</p>
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <Card className="border border-border bg-[#f8f9fa]">
              <CardContent className="p-3">
                ...
                <p className="mt-1 text-lg font-semibold text-orange-600">
                  {stats.currentStreak} días <Timer className="mb-1 ml-1 inline h-4 w-4 text-orange-600" />
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border border-border bg-[#f8f9fa]">
              <CardContent className="p-3">
                <p className="text-xs text-muted-foreground">Días con trabajo</p>
                <p className="mt-1 text-lg font-semibold">
...
export default PomodoroMonthlySidebar;
