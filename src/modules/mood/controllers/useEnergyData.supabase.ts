// src/modules/mo../controllers/useEnergyData.ts
...
export const useEnergyData = (selectedDate: Date) => {
  ...
  const loadEnergyData = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await MoodService.getEnergyEntriesByDate(user.id, dateString);
  ...
  const addEntry = useCallback(async (level: number, comment?: string) => {
    ...
    try {
      const { error: insertError } = await MoodService.insertEnergyEntry({
        userId: user.id,
        date: dateString,
        level,
  ...
  const updateEntry = useCallback(async (originalTimestamp: number, updatedEntry: EnergyEntry) => {
    ...
    try {
      ...
      const { error: updateError } = await MoodService.updateEnergyEntry({
        userId: user.id,
        date: dateString,
        originalTimestamp,
  ...
  const deleteEntry = useCallback(async (timestamp: number) => {
    ...
    try {
      ...
      const { error: deleteError } = await MoodService.deleteEnergyEntry(user.id, dateString, timestamp);
