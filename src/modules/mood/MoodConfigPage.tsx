...
import { moodViews } from '@/modules/mood/views';
...
const MoodConfigPage: React.FC = () => {
  ...
  const loadMoodStates = async () => {
    ...
    setLoading(true);
    const { data, error } = await MoodService.getMoodStates(user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await MoodService.updateMoodState(editingId, {
      ...
      const { error } = await MoodService.createMoodState({
        userId: payload.user_id,
        emoji: payload.emoji,
        text: payload.text,
        value: payload.value,
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await MoodService.deleteMoodState(id);
