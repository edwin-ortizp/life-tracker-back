...
import { Drink, DRINKS } from '../models';
import { getLocalDateString, createFormattedTimestamp } from '@/shared/utils/dates';
...
export const useWaterData = (selectedDate: Date) => {
  ...
  const loadWaterData = async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await WaterService.getDrinkLogs(user.id, dateString);

      if (fetchError) throw fetchError;

      // Transformar a formato esperado
      const drinksList: Drink[] = (data || []).map(row => ({
        type: row.drink_type as keyof typeof DRINKS,
        amount: row.amount,
  ...
  const addDrink = async (type: keyof typeof DRINKS, amount: number) => {
    ...
    try {
      ...
      const { error: insertError } = await WaterService.insertDrinkLog({
        userId: user.id,
        date: dateString,
        drinkType: type,
        amount,
        hydrationValue: hydrationAmount,
  ...
  const editDrink = async (index: number, updatedDrink: Drink) => {
    ...
    try {
      ...
      const { error: updateError } = await WaterService.updateDrinkLog({
        userId: user.id,
        date: dateString,
        originalTimestamp: parseInt(originalDrink.timestamp),
        drinkType: updatedDrink.type,
  ...
  const deleteDrink = async (index: number) => {
    ...
    try {
      ...
      const { error: deleteError } = await WaterService.deleteDrinkLog(
        user.id,
        dateString,
        parseInt(drinkToDelete.timestamp)
      );
