...
import { useExerciseMonthlyProgress, type DailyExerciseAggregate } from '../controllers/useExerciseMonthlyProgress';

interface ExerciseMonthlySidebarProps {
  selectedDate: Date;
...
interface CalendarCell {
  ...
  isFuture: boolean;
  dayNumber: number;
  aggregate: DailyExerciseAggregate;
}
...
const EMPTY_DAY: DailyExerciseAggregate = {
  date: '',
  calories: 0,
  minutes: 0,
  entries: 0,
...
export const ExerciseMonthlySidebar: React.FC<ExerciseMonthlySidebarProps> = ({
  selectedDate,
  onDateChange
}) => {
  const [visibleMonth, setVisibleMonth] = useState(() => startOfMonth(selectedDate));

  const { dailyStatsMap, loading, goal } = useExerciseMonthlyProgress(visibleMonth);
  ...
  }, [dailyStatsMap, goal, monthEnd, monthStart, todayKey, visibleMonth]);
  ...
  const stats = useMemo(() => {
    ...
    const averageMinutes = daysWithData > 0 ? Math.round(totalMinutes / daysWithData) : 0;
      ...
      <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-7 gap-2">
            {calendarCells.map((cell) => {
              ...
              if (cell.aggregate.entries > 0) {
                strokeColor = cell.aggregate.percent > 100 ? 'hsl(var(--primary))' : 'hsl(var(--primary) / 0.75)';
              }
              ...
              return (
                <Tooltip key={cell.dateKey}>
                  ...
                  <TooltipContent side="left" className="max-w-[220px] space-y-1 text-sm">
                    ...
                    <p>{cell.aggregate.calories} / {cell.aggregate.goal} kcal</p>
                    <p>{cell.aggregate.minutes} minutos</p>
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <Card className="border border-border bg-[#f8f9fa]">
              <CardContent className="p-3">
                ...
                <p className="mt-1 text-lg font-semibold text-primary">
                  {stats.currentStreak} días <Flame className="mb-1 ml-1 inline h-4 w-4 text-orange-500" />
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border border-border bg-[#f8f9fa]">
              <CardContent className="p-3">
                <p className="text-xs text-muted-foreground">Días con ejercicio</p>
                <p className="mt-1 text-lg font-semibold">
                  {stats.exerciseDays} / {stats.totalDays}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border border-border bg-[#f8f9fa] sm:col-span-2 lg:col-span-1 xl:col-span-2">
              <CardContent className="p-3">
                ...
                <p className="mt-1 text-lg font-semibold">
                  {stats.averageCalories} kcal | {stats.averageMinutes} min
                </p>
              </CardContent>
            </Card>
          </div>
...
export default ExerciseMonthlySidebar;
