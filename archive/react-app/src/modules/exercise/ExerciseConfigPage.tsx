...
import { exerciseViews } from '@/modules/exercise/views';
...
const ExerciseConfigPage: React.FC = () => {
  ...
  const loadExerciseTypes = async () => {
    ...
    setLoading(true);
    const { data, error } = await ExerciseService.getExerciseTypes(user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await ExerciseService.updateExerciseType(editingId, {
        userId: payload.user_id,
        name: payload.name,
        caloriesPerHour: payload.calories_per_hour,
        stepsEquivalent: payload.steps_equivalent,
      ...
      const { error } = await ExerciseService.createExerciseType({
        userId: payload.user_id,
        name: payload.name,
        caloriesPerHour: payload.calories_per_hour,
        stepsEquivalent: payload.steps_equivalent,
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await ExerciseService.deleteExerciseType(id);
