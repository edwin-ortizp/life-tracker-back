...
import { ShoppingItem, ItemStatus } from '../models';
import { ShoppingListService } from '@/modules/shopping-list/services';
...
export const useShoppingList = () => {
  ...
  const loadShoppingList = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await ShoppingListService.table('shopping_items')
        .select('*')
        .eq('user_id', user.id);
      ...
      const list: ShoppingItem[] = data.map((row: any) => ({
        id: row.id,
        name: row.name || '',
        stock: row.stock || 0,
        toBuy: row.to_buy || 0,
  ...
  const addItem = async (item: Omit<ShoppingItem, 'id'>) => {
    ...
    try {
      ...
      const { error: insertError } = await ShoppingListService.table('shopping_items')
        .insert(docData);
  ...
  const updateItem = async (id: string, data: Partial<ShoppingItem>) => {
    ...
    try {
      ...
      const { error: updateError } = await ShoppingListService.table('shopping_items')
        .update(updateData)
        .eq('id', id);
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      const { error: deleteError } = await ShoppingListService.table('shopping_items')
        .delete()
        .eq('id', id);
