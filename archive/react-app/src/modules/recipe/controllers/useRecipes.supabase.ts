...
export const useRecipes = () => {
  ...
  const loadRecipes = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await RecipeService.table('recipes')
        .select('*')
        .eq('user_id', user.id);

      if (fetchError) throw fetchError;

      const list: Recipe[] = (data || []).map((row: any) => ({
        id: row.id,
        name: row.name,
  ...
  const addRecipe = async (recipe: Omit<Recipe, 'id'>): Promise<string | null> => {
    ...
    try {
      const { data, error: insertError } = await RecipeService.table('recipes')
        .insert({
          user_id: user.id,
          name: recipe.name,
  ...
  const updateRecipe = async (id: string, data: Partial<Recipe>) => {
    ...
    try {
      ...
      const { error: updateError } = await RecipeService.table('recipes')
        .update(updateData)
        .eq('id', id);
  ...
  const deleteRecipe = async (id: string) => {
    ...
    try {
      ...
      const { error: deleteError } = await RecipeService.table('recipes')
        .delete()
        .eq('id', id);
