// src/modules/pomodo../controllers/usePomodoroData.ts
import { useState, useEffect } from 'react';
import { PomodoroService } from '@/modules/pomodoro/services';
import { useAuth } from '@/shared/hooks/useAuth';
import type { PomodoroSession } from '../models';
import { createFormattedTimestamp, getLocalDateString } from '@/shared/utils/dates';
...
export const usePomodoroData = (selectedDate?: Date) => {
  ...
  const loadPomodoroData = async () => {
    ...
    try {
      const { data, error: fetchError } = await PomodoroService.table('pomodoro_sessions')
        .select('*')
        .eq('user_id', user.id)
        .eq('date', date)
      ...
      const pomodoroSessions: PomodoroSession[] = (data || []).map((row: any) => ({
  ...
  const addManualSession = async (options: SaveSessionOptions = {}) => {
    ...
    try {
      ...
      const { error: insertError } = await PomodoroService.table('pomodoro_sessions')
        .insert({
          user_id: user.id,
          date: date,
  ...
  const deleteSession = async (sessionToDelete: PomodoroSession) => {
    ...
    try {
      const { error: deleteError } = await PomodoroService.table('pomodoro_sessions')
        .delete()
        .eq('user_id', user.id)
        .eq('date', date)
  ...
  const editSession = async (
    ...
    try {
      ...
      const { error: updateError } = await PomodoroService.table('pomodoro_sessions')
        .update(updateData)
        .eq('user_id', user.id)
        .eq('date', date)
