// src/modules/mo../controllers/useMoodData.ts
...
export const useMoodData = (selectedDate: Date) => {
  ...
  const loadMoodData = async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await MoodService.getMoodEntriesByDate(user.id, dateString);
  ...
  const addMood = async (mood: { emoji: string; text: string }) => {
    ...
    try {
      ...
      const { error: insertError } = await MoodService.insertMoodEntry({
        userId: user.id,
        date: dateString,
        emoji: newMoodEntry.emoji,
        text: newMoodEntry.text,
        value: newMoodEntry.value ?? getMoodValue(newMoodEntry.text),
  ...
  const updateMood = async (originalTimestamp: number, updatedMood: MoodEntry) => {
    ...
    try {
      ...
      const { error: updateError } = await MoodService.updateMoodEntry({
        userId: user.id,
        date: dateString,
        originalTimestamp,
        emoji: updatedMood.emoji,
        text: updatedMood.text,
        value: updatedMood.value ?? getMoodValue(updatedMood.text),
  ...
  const deleteMood = async (timestamp: number) => {
    ...
    try {
      ...
      const { error: deleteError } = await MoodService.deleteMoodEntry(user.id, dateString, timestamp);
